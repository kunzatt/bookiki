...
public class ChatbotFeedbackController {
    ...
    public ResponseEntity<Page<ChatbotFeedbackEntity>> getFeedbacks(
        ...
        Page<ChatbotFeedbackEntity> feedbacks = chatbotFeedbackService.selectAllFeedbacks(
    ...
    public ResponseEntity<ChatbotFeedbackEntity> selectFeedbackById(@PathVariable int id) {
        ChatbotFeedbackEntity feedback = chatbotFeedbackService.selectFeedbackById(id);
    ...
    public ResponseEntity<ChatbotFeedbackEntity> updateFeedbackStatus(
            @PathVariable int id,
            @RequestBody @Valid UpdateFeedbackStatusRequest request) {

        ChatbotFeedbackEntity updated = chatbotFeedbackService.updateFeedbackStatus(id, request.getStatus());
        return ResponseEntity.status(HttpStatus.OK).body(updated);
    }

    @GetMapping("/user/{userId}")
    @Operation(summary = "사용자별 피드백 조회", description = "특정 사용자의 피드백 내역을 조회합니다.")
    public ResponseEntity<List<ChatbotFeedbackEntity>> selectFeedbackByUserId(@PathVariable int userId) {
        List<ChatbotFeedbackEntity> feedbacks = chatbotFeedbackService.selectFeedbackByUserId(userId);
        return ResponseEntity.status(HttpStatus.OK).body(feedbacks);
    }
