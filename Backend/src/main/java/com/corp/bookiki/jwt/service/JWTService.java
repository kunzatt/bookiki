...
public class JWTService {
    ...
    public boolean isValidRefreshToken(String token, AuthUser user) {
        try {
            String userEmail = extractUserEmail(token);
            ...
            if (isTokenExpired(token)) {
                throw new BusinessException(ErrorCode.TOKEN_EXPIRED,"만료된 리프레시 토큰입니다.");
            }

            String storedToken = redisTemplate.opsForValue().get(REFRESH_TOKEN_PREFIX + user.getEmail());
            if (storedToken == null) {
                throw new BusinessException(ErrorCode.REFRESH_TOKEN_NOT_FOUND,"저장된 리프레시 토큰을 찾을 수 없습니다.");
            }
    ...
    public String generateAccessToken(AuthUser user) { // 액세스 토큰 생성
        return generateToken(user, accessTokenExpire);
    }

    public String generateRefreshToken(AuthUser user) { // 리프레시 토큰 생성 및 Redis에 저장
    ...
    private String generateToken(AuthUser user, long expireTime) { // JWT 토큰 생성 (사용자 정보와 만료 시간 포함)
        return Jwts
                .builder()
    ...
    public Map<String, String> rotateTokens(String oldRefreshToken, AuthUser user) {
        try {
    ...
    public AuthUser getUserInfoFromToken(String token) {
        try {
            ...
            throw new BusinessException(ErrorCode.INVALID_TOKEN,"토큰에서 사용자 정보를 추출할 수 없습니다.");
        }
    }
}
