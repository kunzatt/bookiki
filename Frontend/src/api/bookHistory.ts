...
export const getAdminBookHistories = async (
    ...
    try {
        const response = await axios.get<PageResponse<BookHistoryResponse>>(`${API_URL}/admin/book-histories`, {
            params: {
                periodType,
                startDate,
                endDate,
...
export const getUserBookHistories = async (
    ...
    try {
        const response = await axios.get<PageResponse<BookHistoryResponse>>(`${API_URL}/user/book-histories`, {
            params: {
                periodType,
                startDate,
                endDate,
...
export const getCurrentBorrowedBooks = async (onlyOverdue?: boolean) => {
    try {
        const response = await axios.get<BookHistoryResponse[]>(`${API_URL}/user/book-histories/current`, {
            params: { onlyOverdue }
        });
        return response.data;
    } catch (error) {
