...
export const getBookInformationByIsbn = async (isbn: string): Promise<BookInformationResponse> => {
    try {
        const response = await axios.get<BookInformationResponse>(`${API_URL}/books/info/isbn/${isbn}`);
...
export const getBookInformation = async  (bookInformationId: Number): Promise<BookInformationResponse> => {
    try {
        const response = await axios.get<BookInformationResponse>(`${API_URL}/books/info/${bookInformationId}`);
        return response.data;
    } catch (error) {
